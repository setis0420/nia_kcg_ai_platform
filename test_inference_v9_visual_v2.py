# -*- coding: utf-8 -*-
"""
V9 Direct Coordinate Transformer 추론 및 Folium 시각화 V2
=========================================================
- 울산 100분 예측 모델
- 새로운 전처리 데이터 사용 (울산_100min)
- 선박별 항적 로드
- 60분 간격으로 예측 실행
- Folium 지도에 예측 항적 표시
"""
...
MODEL_DIR = r"k:\coding_project\NIA_선박항적예측프로그램\global_model_v9_ulsan_100_v2"
SEQ_LEN = 50
PRED_LEN = 100
...
def load_model(device):
    ...
    if not os.path.exists(MODEL_DIR):
        ...
        print("학습을 먼저 실행하세요: python run_train_v9_ulsan_100_v2.py")
        return None, None
...
def find_continuous_trajectory(sequences, min_length=200):
    ...
    for i in range(1, len(sequences)):
        ...
        # 이전 시퀀스의 타겟 끝점과 현재 시퀀스의 입력 시작점 비교
...
def run_predictions(model, sequences, device, interval=PREDICTION_INTERVAL):
    ...
    for idx in selected_indices:
        ...
        seq = sequences[idx]
        inp = seq['input'].astype(np.float32)  # (50, 4)
        target = seq['target'].astype(np.float32)  # (100, 2)
...
def create_ship_html(ship_id, predictions, output_path):
    ...
    <div style="position: fixed; bottom: 50px; left: 50px; z-index: 1000;
        ...
        <h4 style="margin: 0 0 10px 0;">V9 울산 100분 예측 V2</h4>
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='V9 울산 100분 예측 시각화 V2 - 단일 선박')
    ...
    parser.add_argument('--num_ships', type=int, default=3, help='생성할 선박 HTML 수')
    args = parser.parse_args()

    print("=" * 60)
    print("V9 Direct Coordinate Transformer - 단일 선박 100분 예측")
    print("=" * 60)
    ...
    if not os.path.exists(DATA_DIR):
        ...
        print("전처리를 먼저 실행하세요: python preprocess_ulsan_v3_100min.py")
        return
    ...
    # 여러 선박에 대해 HTML 생성
    for ship_idx in range(args.num_ships):
        print(f"\n{'='*40}")
        print(f"선박 #{ship_idx + 1} 처리 중...")
        print(f"{'='*40}")

        # 단일 선박의 연속 항적 로드
        sequences = load_single_ship_trajectory(num_chunks=args.num_chunks, ship_idx=ship_idx)

        if not sequences:
            print(f"  [SKIP] 선박 #{ship_idx + 1} - 연속 항적 없음")
            continue
        ...
        if not predictions:
            print(f"  [SKIP] 선박 #{ship_idx + 1} - 예측 없음")
            continue
        ...
        # HTML 생성
        ship_id = f"ship_{ship_idx + 1}"
        output_file = f"prediction_v9_single_{ship_idx + 1}.html"
        create_ship_html(ship_id, predictions, output_file)

        # 오차 통계
        all_errors = np.array([p['errors'] for p in predictions])
        print(f"\n  10분 후: {np.mean(all_errors[:, 9]):.2f} km")
