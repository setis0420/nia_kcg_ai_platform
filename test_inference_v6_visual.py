# -*- coding: utf-8 -*-
"""
V6 H3 Hexagonal Grid 추론 및 시각화
===================================
- H3 셀 기반 예측 결과를 Hexagon으로 시각화
- 실제 항적 vs 예측 항적 비교
"""
...
def generate_html(mmsi, history_points, actual_points, predicted_points, h3_cells_predicted, output_path):
    """HTML 시각화 생성 (Hexagon 표시)"""
...
<head>
    <meta charset="utf-8">
    <title>V6 H3 Hexagonal Grid 예측 - MMSI {mmsi}</title>
    ...
    <style>
        ...
        .legend-hex {{ width: 20px; height: 20px; margin-right: 10px; background: rgba(255,100,100,0.3); border: 2px solid red; }}
    </style>
</head>
<body>
    <div id="map"></div>
    <div class="info-box">
        <h3 style="margin-top:0;">V6 H3 Hexagonal Grid 예측</h3>
        <p><strong>MMSI:</strong> {mmsi}</p>
        <p><strong>H3 Resolution:</strong> {H3_RESOLUTION} (~700m)</p>
        <p><strong>시퀀스 길이:</strong> {SEQ_LEN}분</p>
        <p><strong>예측 시점:</strong> {len(predicted_points)}분 후</p>
        <hr>
        <p><strong>히스토리:</strong> {len(history_points)}개 점</p>
        <p><strong>실제 경로:</strong> {len(actual_points)}개 점</p>
        <p><strong>예측 경로:</strong> {len(predicted_points)}개 점</p>
    </div>
    <div class="legend">
    ...
    <script>
        ...
        var historyLine = L.polyline(historyPoints, {{
            color: 'blue',
            weight: 3,
            opacity: 0.8
        }}).addTo(map);
        ...
        var actualLine = L.polyline(actualPoints, {{
            color: 'green',
            weight: 3,
            opacity: 0.8
        }}).addTo(map);
        ...
        var predictedLine = L.polyline(predictedPoints, {{
            color: 'red',
            weight: 3,
            opacity: 0.8,
            dashArray: '10, 5'
        ...
        if (predictedPoints.length > 0) {{
            L.circleMarker(predictedPoints[0], {{
                radius: 8,
                color: 'orange',
                fillColor: 'orange',
                fillOpacity: 1
...
def run_inference(mmsi):
    ...
    if test_df is None:
        print(f"  [ERROR] 적합한 테스트 데이터 없음 (확인: {checked}개 파일)")
        return
    ...
    with torch.no_grad():
        for step in range(30):
            ...
            x = torch.tensor([cell_indices], dtype=torch.long, device=device)
            logits = model(x)
    ...
    # HTML 생성
    output_path = f"prediction_v6_h3_{mmsi}.html"
    generate_html(mmsi, history_points, actual_points, predicted_points, predicted_cells, output_path)
...
def main():
    print("=" * 60)
    print("V6 H3 Hexagonal Grid 추론 및 시각화")
    print("=" * 60)
