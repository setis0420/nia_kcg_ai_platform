# -*- coding: utf-8 -*-
"""
V13 전국 테스트 - ais_20260206.csv 데이터로 전 지역 20분 예측
============================================================
- 11개 지역 모델 순회
- 예측선 항상 표시, 클릭 시 상세 정보
"""
...
def create_index_html(output_dir, results, base_date, region):
    html = f'''<!DOCTYPE html>
<html><head><title>V13 {region} - {base_date.strftime('%Y-%m-%d')}</title><meta charset="utf-8">
<style>
...
<h1>V13 {region} - {base_date.strftime('%Y-%m-%d')}</h1>
<div class="info"><p><b>예측:</b> {DISPLAY_LEN}분 | <b>파일:</b> {len(results)}개 | <a href="../index.html">전체 목록</a></p></div>
<div class="grid">'''
    for r in results:
        ts = r['time'].replace(':', '')
        imp = r['avg_original'] - r['avg_corrected']
        html += f'''<a class="card" href="prediction_{ts}.html"><div class="time">{r['time']}</div>
<div class="stats">선박: {r['ships']}척<br>오차: <span class="improved">{r['avg_corrected']:.2f} km</span> ({imp:+.2f})</div></a>'''
    html += '</div></body></html>'
...
def create_master_index(root_dir, all_results, base_date):
    html = f'''<!DOCTYPE html>
<html><head><title>V13 전국 - {base_date.strftime('%Y-%m-%d')}</title><meta charset="utf-8">
<style>
body {{ font-family: 'Malgun Gothic', sans-serif; margin: 0; padding: 20px; background: #f5f5f5; }}
h1 {{ color: #2c3e50; border-bottom: 3px solid #e74c3c; padding-bottom: 10px; }}
...
<h1>V13 전국 항적 예측 - {base_date.strftime('%Y-%m-%d')}</h1>
<div class="info"><p><b>데이터:</b> ais_20260206.csv | <b>예측:</b> {DISPLAY_LEN}분 | <b>지역:</b> {len(all_results)}개</p></div>
<div class="grid">'''
...
def main():
    ...
    print("=" * 60)
    print("V13 전국 테스트 - ais_20260206.csv (20분 예측)")
    print("=" * 60)
    ...
    for region, bounds in ALL_REGION_BOUNDS.items():
        ...
        output_dir = os.path.join(base_dir, "예측결과_전국_20260206", region)
        ...
        print(f"\n{'='*60}")
        print(f"[{region}] 데이터: {len(df):,}개, 선박: {df['mmsi'].nunique()}척")
    ...
    root_output = os.path.join(base_dir, "예측결과_전국_20260206")
