# -*- coding: utf-8 -*-
"""
V13 인천 모델 - 30분 단위 전체 예측 HTML 생성
=============================================
...
# V13 예측기 import (모든 필요 클래스/함수 포함)
...
def load_depth_grids_for_display(region='인천'):
    """
...
class PredictorV13WithCorrection:
    ...
    def __init__(self, model_dir, region='인천', device='cuda'):
        self.device = device
        self.region = region
        self.model = None
    ...
    def _init_corrector(self):
        ...
        track_grid_file = os.path.join(script_dir, 'models', 'v12', self.region, 'track_grid.pkl')
...
<body>
    <div class="header">
        <h1>V13 선박 항적 예측 - 인천 (수심/항적 보정)</h1>
...
def main():
    # 스크립트 위치 기준 경로 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_dir = os.path.join(script_dir, "models", "v12", "인천")
    data_path = os.path.join(script_dir, "aisSample.parquet")

    output_dir = os.path.join(script_dir, "예측결과_v13_인천")
    ...
    # 인천 지역 범위
    INCHEON_BOUNDS = {'lat_min': 36.5, 'lat_max': 37.8, 'lon_min': 125.5, 'lon_max': 127.0}

    print("=" * 60)
    print("V13 인천 - 수심/항적 보정 적용 예측")
    print("=" * 60)
    ...
    # V13 예측기 초기화
    predictor = PredictorV13WithCorrection(model_dir, region='인천', device=device)

    # 수심 격자 데이터 로드 (육지 + 얕은 물만)
    depth_grids = load_depth_grids_for_display(region='인천')
    ...
    # 인천 지역 필터링
    df_all = df_all[(df_all['lat'] >= INCHEON_BOUNDS['lat_min']) & (df_all['lat'] <= INCHEON_BOUNDS['lat_max']) &
                    (df_all['lon'] >= INCHEON_BOUNDS['lon_min']) & (df_all['lon'] <= INCHEON_BOUNDS['lon_max'])]

    # 날짜별 처리
    df_all['date'] = df_all['datetime'].dt.date
    unique_dates = sorted(df_all['date'].unique())
    print(f"인천 지역 전체 데이터: {len(df_all):,}개")
    ...
    # 마지막 날짜 데이터로 처리
    df = df_all[df_all['date'] == unique_dates[-1]]
    ...
    for base_time in tqdm(time_slots, desc="HTML 생성"):
        ...
        if ships_data:
            ...
            title = f"V13 인천 예측 - {base_time.strftime('%H:%M')}"
