# -*- coding: utf-8 -*-
"""
V12 Inertia-Aware Transformer - 선종/길이 정보 포함 학습
========================================================
- 입력: (lat, lon, sog, cog) × 30 스텝 + 선종(5) + 길이(9)
- 출력: (Δlat, Δlon) × 60 스텝 (60분 예측)

V12 변경사항 (V11 대비):
- 선종 카테고리 (5개): 화물선, 여객선, 유조선, 예부선, 기타선
- 길이 카테고리 (9개): 40m 단위 (0-40, 40-80, ..., 320m+)
- 메타 정보를 Embedding으로 인코딩하여 모델에 추가
...
# 기본 설정
BASE_DATA_DIR = r"K:\coding_project\NIA_선박항적예측프로그램\학습데이터_전처리\v12"
BASE_SAVE_DIR = r"K:\coding_project\NIA_선박항적예측프로그램\models\v12"
SEQ_LEN = 30   # 입력 시퀀스 길이 (30분)
PRED_LEN = 60  # 예측 길이 (60분)

# 메타 정보 설정
NUM_SHIPTYPE = 5   # 선종 카테고리 수
NUM_LENGTH = 9     # 길이 카테고리 수
META_EMBED_DIM = 16  # 메타 임베딩 차원
...
class TrajectoryTransformerV12(nn.Module):
    ...
    def __init__(self, input_dim=5, hidden_dim=192, num_heads=6,
                 ...
                 num_shiptype=NUM_SHIPTYPE, num_length=NUM_LENGTH,
...
class TrajectoryDatasetV12(Dataset):
    """V12 Dataset (선종/길이 포함)"""
    def __init__(self, sequences):
        ...
        for seq in sequences:
            st = seq.get('shiptype_cat', 4)
            lc = seq.get('length_cat', 0)
            shiptype_counts[st] = shiptype_counts.get(st, 0) + 1
            length_counts[lc] = length_counts.get(lc, 0) + 1

        print(f"  선종 분포: {shiptype_counts}")
        print(f"  길이 분포: {length_counts}")
    ...
    def __getitem__(self, idx):
        ...
        # 메타 정보
        shiptype_cat = sample.get('shiptype_cat', 4)  # 기본: 기타선
        length_cat = sample.get('length_cat', 0)      # 기본: 0-40m
        ...
        return (
            ...
            torch.tensor(shiptype_cat, dtype=torch.long),
            torch.tensor(length_cat, dtype=torch.long)
        )
...
def evaluate_model(model, val_loader, device, pred_len=60):
    ...
    with torch.no_grad():
        for inp, delta_target, _, shiptype, length in val_loader:
            inp = inp.to(device)
            delta_target = delta_target.to(device)
            shiptype = shiptype.to(device)
            length = length.to(device)

            pred_delta = model(inp, shiptype, length)
...
def evaluate_by_category(model, val_loader, device):
    """선종/길이별 오차 분석"""
    model.eval()
    errors_by_shiptype = {i: [] for i in range(NUM_SHIPTYPE)}
    errors_by_length = {i: [] for i in range(NUM_LENGTH)}
    ...
    with torch.no_grad():
        for inp, delta_target, _, shiptype, length in val_loader:
            ...
            pred_delta_np = pred_delta.cpu().numpy() / 100
            target_delta_np = delta_target.cpu().numpy() / 100
            shiptype_np = shiptype.cpu().numpy()
            length_np = length.cpu().numpy()

            for b in range(pred_delta_np.shape[0]):
                # 60분 오차
                dlat = pred_delta_np[b, 59, 0] - target_delta_np[b, 59, 0]
                dlon = pred_delta_np[b, 59, 1] - target_delta_np[b, 59, 1]
                dist_km = np.sqrt((dlat * 111)**2 + (dlon * 111 * np.cos(np.radians(35)))**2)

                st = shiptype_np[b]
                lc = length_np[b]
                errors_by_shiptype[st].append(dist_km)
                errors_by_length[lc].append(dist_km)

    return errors_by_shiptype, errors_by_length
...
def train_model(region, data_dir, save_dir, device, max_chunks=None, epochs=50,
    ...
    """모델 학습"""
    print("=" * 60)
    print(f"V12 Inertia-Aware Transformer - {region} 60분 예측")
    print("=" * 60)
    print(f"Device: {device}")
    print(f"데이터: {data_dir}")
    print(f"저장: {save_dir}")
    print(f"SEQ_LEN: {SEQ_LEN}, PRED_LEN: {PRED_LEN}")
    print(f"선종 카테고리: {NUM_SHIPTYPE}개 {SHIPTYPE_NAMES}")
    print(f"길이 카테고리: {NUM_LENGTH}개")
    ...
    # 3. Dataset 생성
    print("\n[3] Dataset 생성")
    train_dataset = TrajectoryDatasetV12(train_sequences)
    val_dataset = TrajectoryDatasetV12(val_sequences)
    ...
    # 4. 모델 생성
    print("\n[4] 모델 생성")
    model = TrajectoryTransformerV12(
        input_dim=5,
        hidden_dim=192,
        num_heads=6,
        num_layers=4,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for inp, delta_target, last_velocity, shiptype, length in pbar:
            inp = inp.to(device)
            delta_target = delta_target.to(device)
        ...
        with torch.no_grad():
            for inp, delta_target, last_velocity, shiptype, length in val_loader:
                inp = inp.to(device)
                delta_target = delta_target.to(device)
                last_velocity = last_velocity.to(device)
                shiptype = shiptype.to(device)
    ...
    model.load_state_dict(torch.load(os.path.join(save_dir, 'model_best.pth'), weights_only=True))
    errors = evaluate_model(model, val_loader, device, pred_len=PRED_LEN)
    ...
    print("선종별 60분 예측 오차")
    print("-" * 40)
    errors_by_shiptype, errors_by_length = evaluate_by_category(model, val_loader, device)

    for i, name in enumerate(SHIPTYPE_NAMES):
        if errors_by_shiptype[i]:
            avg = np.mean(errors_by_shiptype[i])
            cnt = len(errors_by_shiptype[i])
            print(f"  {name}: {avg:.2f} km ({cnt:,}개)")

    print("\n" + "-" * 40)
    print("길이별 60분 예측 오차")
    print("-" * 40)
    for i, name in enumerate(LENGTH_NAMES):
        if errors_by_length[i]:
            avg = np.mean(errors_by_length[i])
            cnt = len(errors_by_length[i])
            print(f"  {name}: {avg:.2f} km ({cnt:,}개)")
...
def main():
    parser = argparse.ArgumentParser(
        description='V12 Inertia-Aware 60분 예측 모델 - 선종/길이 포함',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
예제:
    python run_train_v12_region.py --list
    python run_train_v12_region.py --region 울산
    python run_train_v12_region.py --region 부산 --epochs 100
        """
    )
    ...
    parser.add_argument('--save_dir', type=str, default=None,
                        help='모델 저장 폴더 (기본: models/v12/지역명)')
    ...
    if args.list:
        ...
        if not regions:
            print("  (없음) - 먼저 preprocess_all_regions_v12.py 실행 필요")
        return
    ...
    if args.data_dir:
        ...
        print("       사용 가능한 지역: python run_train_v12_region.py --list")
        return
    ...
    if not os.path.exists(data_dir):
        ...
        print("       먼저 전처리를 실행하세요:")
        print(f"       python preprocess_all_regions_v12.py --regions {region}")
        return
