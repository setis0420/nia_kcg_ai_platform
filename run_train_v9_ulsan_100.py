# -*- coding: utf-8 -*-
"""
V9 Direct Coordinate Transformer - Ulsan 100분 예측
====================================================
...
SAVE_DIR = "global_model_v9_ulsan_100"
...
class TrajectoryDatasetV9(Dataset):
    """
    V9 Dataset - 100분 예측용
    기존 데이터(20분 타겟)에서 여러 시퀀스를 연결하여 100분 타겟 생성
    """
    def __init__(self, sequences, pred_len=100):
        self.sequences = sequences
        self.pred_len = pred_len
        self.valid_indices = self._find_valid_indices()
        print(f"  유효 시퀀스: {len(self.valid_indices):,}개 (100분 연속 데이터)")

    def _find_valid_indices(self):
        """100분 연속 데이터가 있는 인덱스 찾기"""
        valid_indices = []

        # 위치 기반으로 연속 시퀀스 찾기
        # 기존 데이터는 (50분 입력, 20분 타겟)이므로
        # 100분 타겟을 만들려면 현재 시퀀스 + 이후 4개 시퀀스의 타겟 필요
        # 단, 연속성 검증 필요

        for i in range(len(self.sequences)):
            if i + 4 >= len(self.sequences):
                break

            # 연속 시퀀스인지 확인 (위치 기반)
            seq0 = self.sequences[i]
            is_continuous = True
            ...
            for j in range(1, 5):
                ...
                seq_next = self.sequences[i + j]

                # 이전 시퀀스의 타겟 끝점과 다음 시퀀스의 입력 시작점이 가까워야 함
                prev_target_end = self.sequences[i + j - 1]['target'][-1, :2]
                next_input_start = seq_next['input'][0, :2]

                dist = np.sqrt(np.sum((prev_target_end - next_input_start)**2))
                if dist > 0.01:  # 약 1km 이상 떨어지면 연속 아님
                    is_continuous = False
                    break
    ...
    def __getitem__(self, idx):
        ...
        sample = self.sequences[base_idx]
        inp = sample['input'].astype(np.float32)  # (50, 4)

        # 100분 타겟 생성 (현재 + 이후 4개 시퀀스의 타겟 연결)
        target_list = [sample['target']]  # 첫 20분
        for j in range(1, 5):
            next_seq = self.sequences[base_idx + j]
            target_list.append(next_seq['target'])  # 각 20분씩
...
def train_model(device, max_chunks=None, epochs=50, batch_size=256, lr=1e-3):
    ...
    # 2. Train/Val 분할 (셔플 전에 분할하여 연속성 유지)
    print("\n[2] Train/Val 분할")
    val_size = int(len(sequences) * 0.1)
    train_sequences = sequences[:-val_size]
    val_sequences = sequences[-val_size:]

    print(f"Train 원본: {len(train_sequences):,}, Val 원본: {len(val_sequences):,}")

    # 3. Dataset 생성 (100분 연속 데이터만 사용)
    print("\n[3] Dataset 생성 (100분 연속 시퀀스)")
    train_dataset = TrajectoryDatasetV9(train_sequences, pred_len=PRED_LEN)
    val_dataset = TrajectoryDatasetV9(val_sequences, pred_len=PRED_LEN)

    print(f"Train 유효: {len(train_dataset):,}, Val 유효: {len(val_dataset):,}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='V9 울산 100분 예측 모델 학습')
