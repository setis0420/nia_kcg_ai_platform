"""
울산 항적 데이터 전처리 스크립트 V3
- 100분 예측용 데이터 생성 (pred_len=100)
...
def create_sequences(data, seq_len=50, pred_len=100, step=20):
    ...
    Args:
        data: 전처리된 항적 데이터
        seq_len: 입력 시퀀스 길이 (50분)
        pred_len: 예측 시퀀스 길이 (100분)
        step: 슬라이딩 윈도우 스텝
    """
    sequences = []

    n = len(data['lat'])
    # 100분 예측을 위해 seq_len(50) + pred_len(100) = 150 포인트 필요
...
def process_all_data(input_dir, output_dir, seq_len=50, pred_len=100, step=20):
    """전체 데이터 처리 - 청크 단위로 저장"""
    ...
    stats = {
        ...
        'processed_files': 0,
        'total_sequences': 0,
        'skipped_short': 0,  # 150포인트 미만으로 스킵된 파일
        'shiptype_dist': {},
        'length_dist': {}
    }
    ...
    chunk_idx = 0
    current_chunk = []
    chunk_size = 300000  # 30만 시퀀스마다 저장 (100분 데이터는 더 크므로)
    ...
    print(f"스킵된 파일 (데이터 부족): {stats['skipped_short']}")
...
if __name__ == "__main__":
    ...
    output_dir = r"k:\coding_project\NIA_선박항적예측프로그램\학습데이터_전처리완료\울산_100min"

    process_all_data(
        input_dir=input_dir,
        output_dir=output_dir,
        seq_len=50,      # 입력 시퀀스 길이 (50분)
        pred_len=100,    # 예측 시퀀스 길이 (100분)
        step=20          # 슬라이딩 윈도우 스텝
    )
